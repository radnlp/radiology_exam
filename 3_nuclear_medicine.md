...
- PETにはPET装置, SPECTにはシンチレーションカメラを用いる.
    - PET装置は陽電子が消滅した際に放たれる2本の消滅放射線を検出する.
    - シンチレーションカメラはγ線を1本だけ出す単光子放出核種を検出する.

- シンチレーションカメラの構成
    - コリメータ:
        - 特定の角度で入射されたγ線だけを通し, ほかは遮断する.
        - 材質にはγ線を遮断できる鉛やタングステンを用いる.
    - NaIシンチレータ:
        - 蛍光作用(シンチレーション)を用いた放射線測定装置.
        
- PET装置の構成
    - クリスタル
        - γ線を吸収することによって蛍光作用(シンチレーション)を発揮する物質.
        - 高い阻止能をもつ, 発光量が多く減衰が早い, エネルギー分解能が高いなどの性能が求められる.
            - このうち阻止能は密度と実行原子番号が高くなるほど良好になる.
            - このため, PET装置ではシンチレーションカメラよりも原子番号の大きな物質からなるシンチレータを用いる.
            - Bi4Ge3O12 (BGO), Gd2SiO5:Ce (GSO), Lu2SiO5:Ce (LSO) など.
    - 光検出器
        - 主に光電子倍増管を用いる.
