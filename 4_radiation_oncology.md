...
- ESD/EMRの適応
    - **粘膜固有層まで (T1a-EP, T1a-LPM)** の場合, リンパ節転移の可能性が非常に低いため, **ESD/EMR単独療法の適応**.
    - 粘膜筋板に達する (T1a-MM) か, 粘膜下層にわずかに浸潤(深さ200um未満)の場合, リンパ節転移の可能性はあるため, ESD/EMR単独療法の相対適応.
    - **粘膜下層で浸潤深さが200umを超える**場合, 50%程度の例でリンパ節転移が存在するため, **進行癌に準じた治療**を行う.
...
食道癌の根治的化学放射線療法（処方線量 60 Gy/30 回/6 週）として適切でないのはどれか。1つ選べ。  
`肺の V20 Gy が 50％`  
`肺の最大線量が 62 Gy`  
`PTV の平均線量が 60 Gy`  
`PTV の最低線量が 57 Gy`  
...
`分子標的薬の同時併用が推奨される。`  
`腔内照射で追加照射を行うことが多い。`  
`至適線量は 70 Gy/35 回/7 週程度である。`  

[2016-101](http://www.radiology.jp/content/files/20160825.pdf#page=36)
食道癌（UICC TNM 分類，**第7版**）で領域リンパ節として分類されないのはどれか。1つ選べ。
...
`根治的化学放射線療法では 66 Gy33 回を超えて照射する。`  
`心臓の有害事象は前後対向 2 門照射より 4 門照射で減弱する。`  
